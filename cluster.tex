...
In order to start the operation of cluster system, there are some considerations the must
...
is part of the entire institute which is fenced and guarded by armed security personal.

The meaning of safety in this text is related to both personal safety and also system safety. Personal
safety is not an issue since to special conditions are necessary to operated and use the cluster other
than use an ordinary desktop computer. The safety of the system consist in guarantee environmental conditions
which are not harmful for the hardware. In order to guarantee a proper operational temperature, two dedicated
air conditioning systems work in parallel. These equipments are dimensioned to be able to maintain alone
the working temperature at the cluster room.

Related to reliability, the system is plugged to a remotely configurable no-break of 20KvA. This equipment
can provide power for more than 10 minutes 
